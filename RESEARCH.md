...
- [ ] Research whether Copilot subagents in VSCode fork the context, and start with a clean context. What of the main agent's context is being passed to those subagents? If the memory/context is forked, then this means we can easily utilize subagents for steps like Plan. **Context**: Subagents appear to be defined by agent files but invoked via handoff buttons; unclear if they inherit full conversation history or start fresh.
